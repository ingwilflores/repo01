...
@CrossOrigin()
@RestController
@RequestMapping({ "/api/planDetalle" })
public class PlanDetalleController {
    @Autowired
    private PlanDetalleService servicePrincipal;
    ...
    @PostMapping
    public ResponseEntity<Object> create(@Valid @RequestBody PlanDetalle request){
        PlanDetalle s = servicePrincipal.create(request);
        return new ResponseEntity<>(servicePrincipal.create(request), HttpStatus.OK);
    }
    
    @PutMapping("/{id}")
    public ResponseEntity<Object> update(@PathVariable("id") Long id, @RequestBody PlanDetalle request) {
